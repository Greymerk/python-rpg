...
class Entity:
	...
	def draw(self, screen, posx, posy, textureDir):
		if self.image is None:
			self.image = pygame.image.load(textureDir + self.__class__.living).convert()
		if self.gore is None:
			self.gore = pygame.image.load(textureDir + self.__class__.dead).convert()
		tileSize = 32
